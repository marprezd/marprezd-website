...
Java provides some built-in methods in its standard library that are useful to specify how we want to display the Warningrmation on the console or in the User Graphical Interface. 
. You can use the `System.out.println()` and `System.out.print()` standard methods described in the following code snippet to display Warningrmation to the end-user
